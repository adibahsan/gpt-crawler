        ...
        crawler = new PlaywrightCrawler(
            {
                async requestHandler({request, page, enqueueLinks, log, pushData}) {
                    try {
                        ...
                        await extractAndProcessHtmlContent(page, config, log, pushData, loadedUrl);
                        ...
                        const validLinks = links.filter((link): link is { url: string; type: string } => link !== null);
                        await downloadAndProcessPdfs(validLinks, config, log, pushData, loadedUrl);
                    ...
                    preLaunchHooks: [
                        async () => {
                            ...
                            if (process.platform === 'linux') {
                                await exec('sync && echo 3 | sudo tee /proc/sys/vm/drop_caches');
                            }
                        }
                    ]
                },
